/* upnp scanner, ripped by blackout and modified to evade snort rules */
/* 
 * original code below. is fingerprinted by almost all major network IDS systems, few minor changes can bypass
 *
 * "M-SEARCH * HTTP/1.1\r\nHost:239.255.255.250:1900\r\nST:ssdp:all\r\nMan:\"ssdp:discover\"\r\nMX:3\r\n\r\n";
 */


/* MX:1 appears to work better than MX:3, unsure why but this deserves some research someday,,,, *./
*/
...
void *flood(void *par1)
{
    ...
    int sock;
    if((sock=socket(PF_INET, SOCK_DGRAM, IPPROTO_UDP))<0) { // changed to PF_INET in case we use vps boxes in the future.
        perror("cant open socket. check PF_INET declaration and try AF_INET if dmesg shows errors.");
        exit(-1);
    }
...
void *recievethread()
{
    ...
    if(sock_raw < 0)
    {
        printf("Socket Issue, check source.\n");
        exit(1);
    }
    while(1)
    {
        ...
        if(data_size <0 )
        {
            printf("Recvfrom error , failed to get returned packets\n");
            exit(1);
        }
        struct iphdr *iph = (struct iphdr*)buffer;
        if(iph->protocol == 17) // review this sometime and repurpose this scanner as a protocol fuzzer (targeted, vs protokill.pl)
        {
...
int main(int argc, char *argv[ ])
{
    ...
    memset(temp, 0, 17);
    sprintf(temp, "UPNP Device Found");
    printf("%-16s", temp);
    memset(temp, 0, 17);
    sprintf(temp, "IP/s");
    printf("%-16s", temp);
